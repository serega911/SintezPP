...
const int ari::Viewer::s_colors[] = { WHITE, RED, GREEN, BLUE, YELLOW, ORANGE, PURPLE };

bool isChainsEqual( const NS_CORE Chain& ch1, const NS_CORE Chain& ch2 )
{
...
void ari::Viewer::printKinematicScheme( const KinematicScheme& scheme )
{
	NS_CORE GearBox gearBox( scheme.m_code );
	...
	for ( int row = scheme[0].size()-1; row >= 0; row-- )
	{
		for ( size_t col = 0; col < scheme.size(); col++ )
		{
			...
			{
				int currColorPos = 0;
				if ( scheme[col][row].find( NS_CORE Element::BRAKE ) )
					currColorPos = 1;
				else
				{
